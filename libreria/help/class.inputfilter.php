...
class Request {
    ...
    public function input_html($name) {
        ...
        if (is_array($source)) {
            foreach($source as $key => $value)
                // filter element for XSS and other 'bad' code etc.
                if (is_string($value)) $source[$key] = $this->noremove_html($this->decode($value));
            return $source;
            // clean this string
        } else if (is_string($source)) {
            // filter source for XSS and other 'bad' code etc.
            return $this->noremove_html($this->decode($source));
            // return parameter as given
        } else return $source;
    }
	...
	function filterTags($source) {
		...
		$preTag = NULL;
		$postTag = $source;
		// find initial tag'categorias position
		...
		while($tagOpen_start !== FALSE) {
			...
			// find next tag'categorias start
			$postTag = substr($postTag, ($tagLength + 2));
			$tagOpen_start = strpos($postTag, '<');
		}
	...
	function filterAttr($attrSet) {
		...
		for ($i = 0; $i <count($attrSet); $i++) {
			...
			// auto strip attr'categorias with "javascript:
